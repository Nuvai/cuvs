...
#[derive(Debug)]
pub struct Index<'a> {
    inner: ffi::cuvsIvfPqIndex_t,
    _data: DatasetOwnership<'a>,
}

impl<'a> Index<'a> {
    /// Creates a new FFI index handle.
    fn create_handle() -> Result<ffi::cuvsIvfPqIndex_t> {
        unsafe {
    ...
    /// Builds a new IVF-PQ Index from a borrowed dataset.
    ///
...
impl Index<'static> {
    /// Builds a new IVF-PQ Index from an owned dataset.
    ///
...
impl Drop for Index<'_> {
    fn drop(&mut self) {
        if let Err(e) = check_cuvs(unsafe { ffi::cuvsIvfPqIndexDestroy(self.inner) }) {
...
mod tests {
    ...
    fn test_ivf_pq() {
        ...
        // build the ivf-pq index (owned — device copy lives in the index)
